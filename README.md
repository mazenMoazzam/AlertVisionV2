...
- The architecture has been changed which is reflected upon ths project. Instead of using clips of footage, it has been switched into using public youtube live streams that are accessible. Backend includes the use of FastAPI for processing requests concurrently, and RabbitMQ as the message broker. MongoDB is used to stored detection data from the analyzed feeds for later retrieval from FastAPI for GPT-5 summaries in the frontend.
